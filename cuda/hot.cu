...
void solve_diffusion_2d(const int nx, const int ny, Mesh* mesh,
  ...
  double local_old_r2 =
      initialise_cg(nx, ny, mesh->pad, dt, heat_capacity, conductivity, p, r, x,
                    rho, s_x, s_y, reduce_array, edgedx, edgedy);
  ...
  for (ii = 0; ii < max_inners; ++ii) {
    ...
    const double local_new_r2 =
        calculate_new_r2(nx, ny, mesh->pad, alpha, x, p, r, Ap, reduce_array);
...
double initialise_cg(const int nx, const int ny, const int pad, const double dt,
                     ...
                     double* s_x, double* s_y, double* reduce_array,
                     const double* edgedx, const double* edgedy) {
  ...
  calc_initial_r2<<<nblocks, NTHREADS>>>(nx, ny, pad, s_x, s_y, x, p, r,
                                         reduce_array);
  gpu_check(cudaDeviceSynchronize());
...
double calculate_new_r2(const int nx, const int ny, const int pad, double alpha,
  ...
  int nblocks = ceil(nx * ny / (double)NTHREADS);
  calc_new_r2<<<nblocks, NTHREADS>>>(nx, ny, pad, alpha, x, p, r, Ap,
                                     reduce_array);
  gpu_check(cudaDeviceSynchronize());
